...
module CSVFileOps =
    ...
    let WriteCsvFileResultForContentType csvDir ct filename r =
        File.WriteAllText
            (csvDir + "/" + ct.ToString() + "/" + filename + ".csv", r)
    let WriteCsvFileResultForDate csvDir (date : DateTime) ct r =
        WriteCsvFileResultForContentType csvDir ct 
            (date.ToString("yyyy-MM-dd") + "_" + ct.ToString()) r
    
    let WriteCSVHeaders csvDir pluckTypes getheader =
        for ct in pluckTypes do
            WriteCsvFileResultForContentType csvDir ct "0headers" 
                (getheader (ct) + Environment.NewLine) |> ignore
               ...
               || output.Contains("\n") || output.Contains("\r") then 
                ret <- "\"" 
                       + output.Replace("\"", "\"\"").Replace("\n", " ")
                               .Replace("\r", " ") + "\""
            if ret.Length <= 32767 then ret
            else ret.Substring(0, 32767)
